...
.preview-email-body {
    ...
    border: 1px solid rgba(0,0,0,.08); font-family: sans-serif;
}
...
/* ── SMS char counter ── */
.sms-counter {
    font-size: .72rem; color: var(--text-muted); text-align: right; margin-top: 4px;
}
                    ...
                    <div class="card-body" style="padding:18px;">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" id="emailSubject"
                                   value="Reminder: Your {service} appointment is on {date}"
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" id="emailPreheader"
                                   value="Hi {name}, just a reminder about your upcoming {service}."
                        ...
                        <div class="form-group" style="margin-bottom:0;">
                            ...
                            <div class="editor-toolbar">
                                ...
                                <div class="toolbar-sep"></div>
                                <button class="toolbar-btn" onclick="insertLink()" title="Insert link"><i class="fas fa-link"></i></button>
                                <button class="toolbar-btn" onclick="insertDivider()" title="Insert divider"><i class="fas fa-minus"></i></button>
                                <div class="toolbar-sep"></div>
                                <button class="toolbar-btn" onclick="document.getElementById('emailBody').innerHTML = defaultEmailTemplate()" title="Reset to default" style="font-size:.7rem;padding:5px 10px;">
                    ...
                    <div class="card-body" style="padding:18px;">
                        <div class="form-group" style="margin-bottom:0;">
                            ...
                            <textarea class="form-control" id="smsBody" rows="5"
                                      ...
                                      style="resize:none;font-family:'Barlow',sans-serif;line-height:1.6;"
>Hi {name}! This is a reminder from APX AutoMai. Your {service} appointment is scheduled on {date} at {time}. Please arrive 10 minutes early. Reply STOP to unsubscribe.</textarea>
                ...
                <div class="card-body">
                    <div class="preview-shell">
                        ...
                        <div id="emailPreviewWrap" style="width:600px;max-width:100%;transition:width .3s;">
                            <div class="preview-email-body" id="emailPreviewBody">
                                <!-- Rendered by JS -->
                            </div>
                        </div>
                    </div>
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
            ...
            <div class="card">
                ...
                <div class="card-body" style="padding:14px;">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:6px;">
                        ...
                        <span class="placeholder-chip" title="{{ $desc }}" onclick="insertPlaceholder('{{ $tag }}')">
                            {{ $tag }}
                        </span>
                        @endforeach
                    </div>
            ...
            <div class="card">
                ...
                <div class="card-body" style="padding:14px;display:flex;flex-direction:column;gap:10px;">
                    <div style="font-size:.75rem;color:var(--text-muted);margin-bottom:4px;">
                        Enable which reminders are sent automatically. The cron job runs daily at <strong style="color:var(--text);">8:00 AM</strong>.
                    </div>
                    @php
                        $triggers = [
                            ['key'=>'remind_7d',  'label'=>'7 days before',   'desc'=>'One week reminder',    'icon'=>'fa-calendar-week',   'enabled'=>true],
                            ['key'=>'remind_3d',  'label'=>'3 days before',   'desc'=>'Three day reminder',   'icon'=>'fa-calendar-days',   'enabled'=>true],
                            ['key'=>'remind_1d',  'label'=>'24 hours before', 'desc'=>'Day-of reminder',      'icon'=>'fa-calendar-day',    'enabled'=>true],
                            ['key'=>'remind_2h',  'label'=>'2 hours before',  'desc'=>'Same-day final nudge', 'icon'=>'fa-clock',           'enabled'=>false],
                        ];
                    @endphp
                    ...
                    <div class="trigger-card">
                        ...
                        <label class="trigger-toggle">
                            <input type="checkbox" name="{{ $t['key'] }}" {{ $t['enabled'] ? 'checked' : '' }}>
            ...
            <div class="card">
                ...
                <div class="card-body" style="padding:14px;display:flex;flex-direction:column;gap:12px;">
                    @php
                        $channels = [
                            ['key'=>'send_email', 'label'=>'Send via Email',  'desc'=>'Uses configured SMTP/Gmail', 'enabled'=>true],
                            ['key'=>'send_sms',   'label'=>'Send via SMS',    'desc'=>'Uses configured SMS gateway', 'enabled'=>true],
                        ];
                    @endphp
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
                        ...
                        <label class="trigger-toggle">
                            <input type="checkbox" name="{{ $ch['key'] }}" {{ $ch['enabled'] ? 'checked' : '' }}>
                            <span class="toggle-slider"></span>
                        </label>
                    </div>
                    @endforeach
...
@push('scripts')
<script>
// csrf token exposed before verbatim block
    ...
    <div style="padding:32px;">
      ...
      <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:20px;margin:0 0 24px;">
        ...
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            <td style="padding:6px 0;font-size:.83rem;color:#6b7280;">Service</td>
            <td style="padding:6px 0;font-size:.83rem;font-weight:600;color:#111;text-align:right;">{service}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;font-size:.83rem;color:#6b7280;border-top:1px solid #f3f4f6;">Date</td>
            <td style="padding:6px 0;font-size:.83rem;font-weight:600;color:#111;text-align:right;border-top:1px solid #f3f4f6;">{date}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;font-size:.83rem;color:#6b7280;border-top:1px solid #f3f4f6;">Time</td>
            <td style="padding:6px 0;font-size:.83rem;font-weight:600;color:#111;text-align:right;border-top:1px solid #f3f4f6;">{time}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;font-size:.83rem;color:#6b7280;border-top:1px solid #f3f4f6;">Vehicle</td>
            <td style="padding:6px 0;font-size:.83rem;font-weight:600;color:#111;text-align:right;border-top:1px solid #f3f4f6;">{plate} — {vehicle}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;font-size:.83rem;color:#6b7280;border-top:1px solid #f3f4f6;">Reference</td>
            <td style="padding:6px 0;font-size:.83rem;font-weight:600;color:#E8192C;text-align:right;border-top:1px solid #f3f4f6;">{reference}</td>
          </tr>
        </table>
      </div>
      ...
      <div style="text-align:center;margin:0 0 24px;">
        <a href="#" style="display:inline-block;background:#E8192C;color:#fff;padding:12px 28px;border-radius:7px;text-decoration:none;font-weight:700;font-size:.88rem;letter-spacing:.02em;">
          View My Booking
        </a>
      </div>
...
function saveAllTemplates() {
    ...
    fetch('{{ route("admin.templates.save") }}', {
        method: 'POST',
        headers: {
